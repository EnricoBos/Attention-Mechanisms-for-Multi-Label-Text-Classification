...
class BahdanauAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        # Initialize the context vector [dim,1]
...
class LuongAttentionForClassification(Layer):
    def __init__(self, return_sequences=False, **kwargs):
        super(LuongAttentionForClassification, self).__init__(**kwargs)
        self.return_sequences = return_sequences

    def build(self, input_shape):
        # Infer embedding_dim from input shape
        self.embedding_dim = input_shape[-1]  # The last dimension of input shape corresponds to the embedding dimension
        
        # Define a trainable query vector using the inferred embedding_dim
        self.query_vector = self.add_weight(
            shape=(self.embedding_dim, 1),  # Shape is [embedding_dim, 1]
            initializer='glorot_uniform',
            trainable=True,
            name='query_vector'
        )
    ...
    def call(self, inputs):
        ...
        # Perform matmul between inputs [N, SL, E] and query vector [E, 1]
        eij = tf.matmul(inputs, self.query_vector)  # Resulting shape: [N, SL, 1]
        
        # Squeeze the last dimension to get [N, SL] attention scores
        eij = tf.squeeze(eij, axis=-1)
        
        # Apply softmax to get the attention weights
        attention_weights = tf.nn.softmax(eij, axis=1)  # Shape: [N, SL]
        
        # Expand dimensions to [N, SL, 1] to match inputs
        attention_weights = tf.expand_dims(attention_weights, axis=-1)
        
        # Compute the context vector as the weighted sum of the inputs
        weighted_inputs = inputs * attention_weights  # Shape: [N, SL, E]

        if self.return_sequences:
            return weighted_inputs  # Return [N, SL, E]

        # Return the attention-weighted sentence representation (context vector)
        context_vector = tf.reduce_sum(weighted_inputs, axis=1)  # Shape: [N, E]
        return context_vector

    def compute_output_shape(self, input_shape):
        if self.return_sequences:
            return input_shape  # Return [N, SL, E]
        return (input_shape[0], input_shape[-1])  # Return [N, E]

    def get_config(self):
        config = super(LuongAttentionForClassification, self).get_config()
        config.update({
            'return_sequences': self.return_sequences
        })
        return config
...
def build_model(maxlen, vocab_size, embedding_size, embedding_matrix, target_count,model_type):
    if(model_type == 'no_attention'):
    ...
    elif(model_type == 'luong_attention'):
        print('luong_attention enabled')
        # Input layer for words
        input_words = Input(shape=(maxlen,))
        ...
        #x_words = LSTM(150, return_sequences=True)(x_words)
        # Apply Attention mechanism
        x = LuongAttentionForClassification(return_sequences=True)(x_words)
        #x = Attention()([x_words, x_words]) 
        ###########################################################################
        # Apply Dropout
        x = Dropout(0.2)(x)
...
if __name__ == "__main__":
    ...
    ##########################################################################
        ...
        if(model_type == 'no_attention'):
            ...
            try:
                model = load_model('best_model_'+model_type+ '.h5', custom_objects={'LuongAttentionForClassification': LuongAttentionForClassification})
            except:
                print('no model found..')
                sys.exit()  # Exit the script
